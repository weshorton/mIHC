...
uniqCol_v <- params$uniqCol
metaCols_v <- strsplit(params$metaCols, split = ",")[[1]]

### Make sure patients are characters
meta_dt[[uniqCol_v]] <- as.character(meta_dt[[uniqCol_v]])
cell_dt[[uniqCol_v]] <- as.character(cell_dt[[uniqCol_v]])

if ("timepoint" %in% colnames(meta_dt)) {meta_dt$timepoint <- as.character(meta_dt$timepoint)}
```

```{r temporaryColorStuff, echo = F, eval = F}
...
### Subset metadata for given sample and also subset columns
### If no pt_v provided, then don't subset.
if (!is.null(pt_v)) {
  subMeta_dt <- unique(meta_dt[get(idCol_v) == pt_v,mget(metaCols_v)])
} else {
  subMeta_dt <- unique(meta_dt[,mget(metaCols_v)])
}

### This if for AMTEC only
if ("timepoint" %in% colnames(subMeta_dt)) {
  ### add bx to timepoint (for biopsy)
...
### To avoid this hardcode, need to generalize the columns better...
# if (prj_v == "MdR01") {
#   subCell_dt <- subCell_dt[,Sample_ID := subMeta_dt[match(subMeta_dt$Sample_ID, Sample_ID),paste(Tumor_ID, Subregion, sep = "-")]]
#   colnames(subCell_dt)[colnames(subCell_dt) == "Sample_ID"] <- "Tumor_ID"
#   subMeta_dt[,Tumor_ID := paste(Tumor_ID, Subregion, sep = "-")]
# }

### Subset data
subCell_dt <- cell_dt[get(uniqCol_v) %in% subMeta_dt[[uniqCol_v]],]
subFunc_lsdt <- func_lsdt[subMeta_dt[[uniqCol_v]]]
...
### Check that unique column is the same
metaClass_v <- class(subMeta_dt[[uniqCol_v]])
dataClass_v <- class(subCell_dt[[uniqCol_v]])

if (metaClass_v != dataClass_v) {
  cat(sprintf("uniqCol: %s has mismatched classes.\n%s in meta\n%s in data.\nChanging both to character.\n",
              uniqCol_v, metaClass_v, dataClass_v))
  subMeta_dt[[uniqCol_v]] <- as.character(subMeta_dt[[uniqCol_v]])
  subCell_dt[[uniqCol_v]] <- as.character(subCell_dt[[uniqCol_v]])
}
...
if ("stackedBar" %in% names(plotCols_lsv)) {
  ...
  for (i in 1:length(plotCols_lsv$stacked)) {
    ...
    if ("timepoint" %in% colnames(subMeta_dt)) {
      mergeCols_v <- c(uniqCol_v, "timepoint")
    } else {
      mergeCols_v <- uniqCol_v
    }
    
    currData_dt <- merge(subMeta_dt[,mget(mergeCols_v)],
                         subCell_dt[,mget(c(uniqCol_v, currConfig_dt$Class))], 
                         by.x = uniqCol_v, by.y = uniqCol_v, sort = F)
    
    if ("timepoint" %in% colnames(subMeta_dt)) setkey(currData_dt, "timepoint") # sort by timepoint
    ...
    ### Assign x variable
    if ("timepoint" %in% colnames(subMeta_dt)) { x_v <- "timepoint" } else { x_v <- uniqCol_v}
...
if ("pie" %in% names(plotCols_lsv)) {
  ...
  currData_df <- convertDFT(subCell_dt[,mget(c(uniqCol_v, currConfig_dt$Class))], col_v = uniqCol_v)
  ...
  currData_dt <- merge(subMeta_dt[,mget(mergeCols_v)],
                       convertDFT(currData_df, newName_v = uniqCol_v),
                       by = uniqCol_v, sort = F)
  
  if ("timepoint" %in% colnames(subMeta_dt)) setkey(currData_dt, "timepoint")
...
if ("bar" %in% names(plotCols_lsv)) {
  ...
  currFunc_lsdt <- sapply(names(subFunc_lsdt), function(x) {
    ...
    if ("timepoint" %in% colnames(subMeta_dt)) {
      yy$timepoint <- subMeta_dt[get(uniqCol_v) == x,timepoint]
    } else {
      yy$key <- x
    }
    return(yy)
  ...
  if ("timepoint" %in% colnames(currFunc_dt)) {
    setkey(currFunc_dt, "timepoint")
  } else {
    setkey(currFunc_dt, cols = "key")
  }
  
  ### Melt - this is ugly
  if ("timepoint" %in% colnames(currFunc_dt)) { meltCols_v <- c("Class", "timepoint") } else { meltCols_v <- c("Class", "key")}
  #if (prj_v == "MdR01") { meltCols_v <- c("Class", uniqCol_v) } else {meltCols_v <- "Class"}
  ...
  ### Add Facet
  if ("timepoint" %in% colnames(subMeta_dt)) {
    curr_gg <- curr_gg + facet_wrap(~timepoint)
  } # fi
  
  if (prj_v == "MdR01") {
    curr_gg <- curr_gg + facet_wrap(~key)
  } # fi
