calcDensity <- function(data_dt, meta_dt = NULL, areaCol_v = "Area", idCol_v = "Sample_ID", metaCols_v = NULL) {
  ...
  ### If both have one, make sure they're the same value and remove
  ### If not in data, make sure it is in meta.data. Then merge
    ...
    if ("Area" %in% colnames(data_dt)) {
      ...
      if (!is.logical(checkArea_v)) {
        stop("Area columns in provided data and metadata don't match. Make sure they do!\n")
      } else {
        cat(sprintf("Area columns in data and meta match. Removing from data"))
        data_dt[[areaCol_v]] <- NULL
      }
    } else {
    ...
    data_dt <- merge(data_dt, meta_dt[,mget(c(idCol_v, areaCol_v)),drop = F], by = idCol_v, sort = F)
